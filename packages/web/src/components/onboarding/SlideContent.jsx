...
/**
 * Composant harmonisé pour l'affichage du contenu des slides
 * Gère tous les types : features, steps, options, process, rewards
 */
export default function SlideContent({ slideData, currentSlide }) {
  ...
  // Reset animation lors du changement de slide
  useEffect(() => {
    setCurrentItem(0);
  }, [currentSlide]);
        ...
        <div className="space-y-1 mt-3">
          {slideData.features.map((feature, index) => (
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ delay: (slideData.items.length + index) * 0.2 }}
              className="text-white text-sm bg-black/70 backdrop-blur-sm rounded-lg p-2 shadow-lg drop-shadow-md text-center"
            >
              {feature}
            </motion.div>
          ))}
